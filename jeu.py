...
from display import *
from ia import ia_play  # Import de l'IA
from ai_trainer import AITrainer  # Import du système d'entraînement
...
# Variables pour l'IA
ai_delay = 1000  # Délai en millisecondes entre chaque action IA
last_ai_action = 0
ai_units_to_process = []
ai_processing = False
...
clock = pygame.time.Clock()  # Pour gérer le timing de l'IA
...
while running:
    ...
    if not victory:
                        ...
                        if 0 <= grid_x < size and 0 <= grid_y < size:
                            if event.button == 1:
                                possible_units = [u for u in units if
                                                  u.x == grid_x and u.y == grid_y and not u.moved and u.color == (
        ...
        # Gestion de l'IA (tour ennemi)
            ...
            # Initialiser les unités IA à traiter si pas encore fait
            ...
            if ai_processing and ai_units_to_process and current_time - last_ai_action >= ai_delay:
                ...
                # Faire jouer l'IA pour cette unité
                action, target = ia_play(current_ai_unit, units, objectives, size)

                # Enregistrer l'action pour l'entraînement
        ...
        if unit_moved:
            ...
            units_to_move = [unit for unit in units if
                             (unit.color == PLAYER_COLOR if player_turn else unit.color == ENEMY_COLOR)]
